...
const BLOCK_KM = 2;
const WHATSAPP_NUMBER = '7477921430';
...
function createOsmRouteLink(fromLat, fromLng, toLat, toLng) {
...
function generateWhatsAppMessage() {
  ...
  if (priceInfo) {
    ...
    // Готовый маршрут
    message += `🗺️ *ГОТОВЫЙ МАРШРУТ:*\n`;
    message += `Google Maps: ${createGoogleMapsRouteLink(a.lat, a.lng, b.lat, b.lng)}\n`;
    message += `OpenStreetMap: ${createOsmRouteLink(a.lat, a.lng, b.lat, b.lng)}\n\n`;
  }
  
  // Информация о файлах
  const files = document.getElementById('photo').files;
  if (files.length > 0) {
    message += `📷 *ФОТО ТОВАРА:* ${files.length} файл(ов)\n\n`;
  }
